...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    val sslContext: SSLContext by lazy(LazyThreadSafetyMode.NONE) {
        // Use bundled Conscrypt provider so exportKeyingMaterial works on all API levels
        val provider = try {
            Class.forName("org.conscrypt.Conscrypt").getMethod("newProvider").invoke(null) as java.security.Provider
