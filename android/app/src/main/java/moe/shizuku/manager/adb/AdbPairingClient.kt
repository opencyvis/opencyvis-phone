...
class AdbPairingClient(private val host: String, private val port: Int, private val pairCode: String, private val key: AdbKey) : Closeable {
    ...
    companion object {
        ...
        private fun exportKeyingMaterial(
            sslSocket: SSLSocket, label: String, context: ByteArray?, length: Int
        ): ByteArray {
            // Use bundled org.conscrypt directly (no reflection needed)
            return org.conscrypt.Conscrypt.exportKeyingMaterial(sslSocket, label, context, length)
        }
    }
}
